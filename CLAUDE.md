...
When `literature_search.enabled: true` in `config.yaml`, the system builds a 3-paper pack from `citation_graph/output/graph_state.json` and generates questions. The strategy depends on the LLM provider:
...
```bash
cd citation_graph && python enrich_arxiv_ids.py
cd citation_graph && python enrich_missing_urls.py  # S2 batch + arxiv title search for remaining gaps
```
...
Agent logic lives in `citation_graph/literature_search/agent_questions.py`.

**Smoke-tested:** The agent subprocess path (Anthropic/`claude`) has been verified end-to-end — prompt substitution, subprocess invocation, PDF reading by the agent, `Research_Questions.txt` creation, and content retrieval all work. The full pipeline path requires `citation_graph/output/graph_state.json` to exist (build it with `python citation_graph/cli.py build`).

### Question Prioritizer Agent Mode (`src/questions/agent_prioritizer.py`)
...
Agent logic lives in `src/questions/agent_prioritizer.py`. Prompt template in `prompts/agent_prioritizer.yaml`.

### Analysis Agent Mode (`src/analysis/agent_analysis.py`)
...
Agent logic lives in `src/analysis/agent_analysis.py`. Prompt template in `prompts/agent_analysis.yaml`.

### Container Runtime (Docker / Podman)

The legacy analysis pipeline executes AI-generated scripts inside a container sandbox when `analysis.execution.sandbox: true` and `execution.mode: "sandbox"`. Both Docker and Podman are supported as container runtimes.
...
Runtime detection (`runtime` key):
- `"auto"` (default) — checks `docker` first, then `podman`
- `"docker"` / `"podman"` — uses only that binary
- Result is cached in `config["execution"]["docker"]["_resolved_runtime"]` by `check_docker_sandbox()`

Podman-specific adjustments (handled automatically):
- **User namespace**: `--userns=keep-id` instead of `--user UID:GID`
- **GPU access**: `--device nvidia.com/gpu=all` (CDI) instead of `--gpus all`
- **SELinux**: `:z` suffix appended to bind mounts when SELinux is enforcing (Fedora/RHEL)

All sandbox logic lives in `src/analysis/analysis_executor.py`.

### Visualization Agent Mode (`src/visualization/agent_visualization.py`)
...
Agent logic lives in `src/visualization/agent_visualization.py`. Prompt template in `prompts/agent_visualization.yaml`.

### Report Agent Mode (`src/reporting/agent_report.py`)
...
Agent logic lives in `src/reporting/agent_report.py`. Prompt template in `prompts/agent_report.yaml`.

### AutoCritique Agent Mode (`src/autocritique/agent_autocritique.py`)
...
**Verdict format:** The agent is instructed to include one of three exact sentinel strings in its review: `Verdict: Reject`, `Verdict: Revise and Resubmit`, or `Verdict: Accept`. The pipeline parses these strings programmatically to decide next steps. The prompt stresses that one of these exact strings must appear verbatim.

**Reject handling:** If the final verdict is `Reject`, the pipeline renames the project directory with a `REJECT_` prefix (e.g. `REJECT_my_study_2026-03-17T10-00-00`) before proceeding to repo assembly and notebook generation. The rename updates `PathResolver`, `config["project_id"]`, and `PipelineUI` so downstream steps still resolve paths correctly.
...
Agent logic lives in `src/autocritique/agent_autocritique.py`. Prompt template in `prompts/agent_autocritique.yaml`. Toggleable via Options menu (#12).

### Revision Agent Mode (`src/autocritique/agent_revision.py`)
...
Agent logic lives in `src/autocritique/agent_revision.py`. Prompt template in `prompts/agent_revision.yaml`.

### Report Revision Agent Mode (`src/reporting/agent_report_revision.py`)
...
Agent logic lives in `src/reporting/agent_report_revision.py`. Prompt template in `prompts/agent_report_revision.yaml`.

### Repo Agent Mode (`src/repo/agent_repo.py`)
...
Agent logic lives in `src/repo/agent_repo.py`. Prompt template in `prompts/agent_repo.yaml`.

### Notebook Agent Mode (`src/notebook/agent_notebook.py`)
...
Agent logic lives in `src/notebook/agent_notebook.py`. Prompt template in `prompts/agent_notebook.yaml`.
...
|-------|-------------|--------|-------------------|
| `questions` | `src/questions/agent_questions.py` | `questions/` | `literature/` (optional) |
| `viz` | `src/visualization/agent_visualization.py` | `visualizations/` | `analysis/`, `questions/` |
| `report` | `src/reporting/agent_report.py` | `reports/` | `analysis/`, `visualizations/`, `questions/` |
| `notebook` | `src/notebook/agent_notebook.py` | `repo/notebooks/` | `repo/` subdirs (symlinked individually) |
...
### Agent Subprocess Progress Polling (`src/core/agent_subprocess.py`)
...
### Pipeline UI (`src/core/pipeline_ui.py` + `src/core/dashboard_template.py`)
...
### Interactive Mode (`src/core/interactive.py`)
...
Revision prompts live in `prompts/interactive.yaml`. Core helpers in `src/core/interactive.py`: `is_interactive()`, `format_stage_output()`, `interactive_checkpoint()`, `make_revision_call()`.
...
| `src/core/pipeline_ui.py` | `PipelineUI` class — step tracking, LLM interaction recording, progress messages, HTML dashboard |
| `src/core/dashboard_template.py` | HTML template, CSS, JS, and render helpers for the dashboard |
| `src/core/utils.py` | `PathResolver` singleton, utilities |
...
| `src/questions/question_manager.py` | Question file I/O, prioritization |
| `citation_graph/literature_search/` | Literature search: sampling, download, agent questions, run |
| `citation_graph/literature_search/agent_questions.py` | CLI agent subprocess invocation |
| `citation_graph/literature_search/run.py` | Literature search orchestration + LLM fallback |
| `citation_graph/literature_search/download.py` | Article download (PDF + HTML) + manifest writing |
| `prompts/question_manager.yaml` | All question prompts (generator, prioritizer, agent question generator) |
| `src/questions/agent_prioritizer.py` | CLI agent question prioritization: subprocess, output reading |
| `prompts/agent_prioritizer.yaml` | Prompt template for prioritizer agent |
| `src/analysis/agent_analysis.py` | CLI agent analysis: workspace setup, subprocess, output reading |
| `prompts/agent_analysis.yaml` | Prompt template for analysis agent iterations |
| `src/visualization/agent_visualization.py` | CLI agent visualization: subprocess, output reading |
| `prompts/agent_visualization.yaml` | Prompt template for visualization agent |
| `src/reporting/agent_report.py` | CLI agent report generation: subprocess, output reading |
| `prompts/agent_report.yaml` | Prompt template for report agent |
| `src/autocritique/agent_autocritique.py` | CLI agent autocritique: subprocess, output reading |
| `prompts/agent_autocritique.yaml` | Prompt template for autocritique agent |
| `src/autocritique/agent_revision.py` | CLI agent revision: per-recommendation subprocess, output reading |
| `prompts/agent_revision.yaml` | Prompt template for revision agent |
| `src/reporting/agent_report_revision.py` | CLI agent report revision: incorporates revisions into report |
| `prompts/agent_report_revision.yaml` | Prompt template for report revision agent |
| `src/repo/agent_repo.py` | CLI agent repo assembly: finalized files into clean repo structure |
| `prompts/agent_repo.yaml` | Prompt template for repo agent |
| `src/notebook/agent_notebook.py` | CLI agent notebook generation: creates Jupyter notebook from repo |
| `prompts/agent_notebook.yaml` | Prompt template for notebook agent |
| `citation_graph/enrich_arxiv_ids.py` | Batch-enrich graph with arxiv_id + open_access_url |
...
- The `citation_graph` directory is added to `sys.path` at runtime so its subpackages can be imported directly (e.g., `from literature_search.run import ...`)
